...
rgbarray=[]
print(len(pixel)-5737728)
for x in range(717216,len(pixel)):
    i = (pixel[x].dot(2 ** np.arange(14)[::-1]))
    rgbarray.append(int(round((i/16383)*255)))  #int(round((i/16383)*255))    ho creato un array dove ogni elemento rappresenta i 14 bit convertiti in valore RGB
...
7058
#5280*3529
distance = [[[0 for k in range(3)] for j in range(5184)] for i in range(3456)] #creo array di 0

distance=np.array(distance)
B = np.reshape(rgbarray, (-1, 5184))    #trasformo rgbarray in un array 2D per capire quando andare alla riga sotto
print(B)
arrayred=[]
arraygreen=[]
arrayblue=[]
